...
import { validateEmailFormat, validatePasswordFormat, isPasswordMatched } from '../../utils/validations.js'
...
const Register = () => {
  ...
  const initialForm = { 'email': '', 'password': '', 'confirmedPassword': '' }
  const initialErr = { 'email': '', 'password': '', 'confirmedPassword': '', isValid: false }
  ...
  const validateCopy = (form) => {
    const validate = isPasswordMatched(form.password, form.confirmedPassword);
    if (validate) setErr({ ...err, confirmedPassword: '', isValid: true });
    if (!validate) setErr({ ...err, confirmedPassword: 'Password mismatch', isValid: false });
    if (!form.confirmedPassword) setErr({ ...err, confirmedPassword: 'Required field', isValid: false })
  }
  const handleFormChange = (e) => {
    ...
    name === 'confirmedPassword' && validateCopy({
      ...form,
      [name]: value
    });
      ...
      <div className="p-4 flex items-center justify-center h-[75vh]">
        {loading ?
          ...
          <div className="regForm">
            <h1 className="text-lg font-bold text-center mb-2">REGISTER</h1>
            <form onSubmit={handleSubmit}>
              <label className="input input-bordered flex items-center gap-2 min-w-[30vw]">
                ...
                <input name='email' value={form.name} type="text" className="grow" placeholder="Email" onChange={handleFormChange} />
                <br />
              </label>
              ...
              <label className="input input-bordered flex items-center gap-2 min-w-[30vw]">
                ...
                <input type="password" value={form.password} name='password' className="grow" placeholder="password" onChange={handleFormChange} />
              </label>
              ...
              <label className="input input-bordered flex items-center gap-2 min-w-[30vw]">
                ...
                <input type="password" className="grow" placeholder="confirm password" name='confirmedPassword' value={form.confirmedPassword} onChange={handleFormChange} />
              </label>
              <p className='text-orange-500 ml-2 min-h-8'>{err.confirmedPassword}</p>
